...
- **Agent 密钥管理**：后台网站设置新增 MCP 密钥管理，支持读/写权限勾选、密钥掩码显示、一键复制、重置/吊销二次确认、最多 2 个密钥、写操作审计日志
- **图片管理**：后台新增「图片管理」页面，展示 R2 存储桶全部图片（含文章封面图），支持上传、删除、复制链接、显示图片尺寸
- **编辑器插入图片**：编辑器新增「上传图片」按钮，弹窗展示/上传 R2 图片，一键生成 Markdown 插入正文；上传与封面图自动进入图片管理
- **文章目录（TOC）**：文章含 ≥2 个二级/三级标题时自动生成目录卡片，点击平滑跳转；后台「个性设置」可开关

#### 界面优化
- 后台导航原「个人设置」「外观布局」两个独立页面合并为单个「个性设置」页面：左侧「个人信息」卡片（个人名称、头像、简介、建站时间、友链标题/图标/内容），右侧「布局与模块」卡片（主题风格、个人简介位置、标签云/文章目录开关与位置、图标、置顶文章、广告位置与内容）；PC 双列、手机单列
- 后台各设置项改为「标题 | 内容」横向布局（移动端自动回退上下布局）
- 设置页输入框/文本域样式与文章编辑器统一（胶囊圆角、描边、聚焦高亮）
- 图片管理预览容器加大
- 文章页仅在正文内容区显示输入光标

#### 问题修复
- 修复文章页中等屏幕（769–1096px）横向溢出，内容区改为弹性收缩
- 修复移动端无左侧栏时汉堡按钮崩溃、无侧栏时重复渲染导航按钮
- 修复后台新建文章默认「草稿」导致前台打不开（默认改为「已发布」）
- 兼容 WordPress 导入/旧版本遗留的 `publish` 状态（前台全部查询同步适配）
- 修复旧版日期式文章链接（如 `/post/202607/1`）404，自动 301 跳转到规范链接 `/post/1`
- 404 响应增加 `Cache-Control: no-store`，避免 CDN/浏览器缓存旧错误页

#### 精简
- 移除文章页浏览量展示及底层 view_count 统计代码
- 移除文章页更新日期/修改时间展示及相关 SEO 元数据（article:modified_time、dateModified）
- 移除文章页预计阅读时长
...
<details>
<summary>v1.1.0 (2026-07-05)</summary>
